...
function(extract_file filename extract_dir)
  ...
  # 创建临时目录，用来解压，如果已经存在，则删除 这里用的解压命令，是 cmake 内部实现的解压命令，可以实现跨平台解压： cmake -E tar
  # -xf filename.tgz
  ...
  # 也可能是直接就包含着我们需要的内容，因此，这里就统一处理，如果包含单独的文件夹 则将安装源目录设置为临时目录的下级目录
...
function(download_and_extract)
  ...
  if(EXISTS ${DAE_FILENAME})
    ...
    message(STATUS "Download URL: ${DAE_URL}")
    message(STATUS "Download FileName: ${DAE_FILENAME}")
    message(STATUS "Download HASH Type: ${DAE_HASH_TYPE}")
    message(STATUS "Download HASH: ${DAE_HASH}")
    download_file_with_hash(${DAE_URL} ${DAE_FILENAME} ${DAE_HASH_TYPE}
                            ${DAE_HASH})
  endif()

  if (EXISTS ${DAE_FILENAME})
    message(STATUS "Using extract_file ${DAE_FILENAME}")
